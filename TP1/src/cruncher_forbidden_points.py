...
if __name__ == "__main__":
    ...
    with open(CSV, 'w', newline='') as csvfile:
        ...
        csv_writer.writerow(['Map', 'Steps', 'Algorithm', 'Heuristic', 'Visited Count', 'End State Steps', 'Frontier count', 'Execution Time', 'Forbidden points'])
            ...
            for algorithm_name in config["algorithm"]:
                ...
                if algorithm_name in ["BFS","DFS"]:
                    for i in range(0, repetitions):
                        ...
                        csv_writer.writerow([map_path, steps, algorithm_name, 'NONE', visited_count, end_state_steps, frontier_count, execution_time, config["forbidden_points"]])
                else:
                    for heuristic_name in config["heuristic"]:
                        for i in range (0, repetitions):
                            ...
                            print(f"    Result: Visited Count = {visited_count}, End State Steps = {end_state_steps}, Execution Time = {execution_time:.2f}")
                            csv_writer.writerow([map_path, steps, algorithm_name, heuristic_name, visited_count, end_state_steps, frontier_count, execution_time, config["forbidden_points"]])
